export default async () => {
  try {
    ...
    const githubRes = await fetch('https://api.github.com/repos/nhsuk/nhsuk-frontend/releases');
    ...
    const promises = releases.map(async (release, index) => {
      ...
      const npmRes = await fetch(`https://api.npmjs.org/downloads/point/${release.published_at.split('T')[0]}:${end.split('T')[0]}/nhsuk-frontend`);
